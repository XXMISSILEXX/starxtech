...
def _acl(user, album, action):
    if user.role_code in ADMIN_ROLES or not album.is_restricted: return True
...
def _can(user, album, code, action, archived=False): return bool(album and (archived or (album.is_active and not album.deleted_at)) and access(user) and user.can(code) and _acl(user, album, action))
