...
from app.auth.permissions import can_edit_progress_entry, progress_entry_required, progress_read_required, progress_structure_required
...
def project_progress(project_id):
    return jsonify({"types": [{"id": item.id, "name": item.name} for item in _project(project_id).progress_types]})
...
def create_type(project_id):
    ...
    db.session.commit(); return jsonify({"id": value.id}), 201
...
def edit_type(project_id, type_id):
    ...
    db.session.commit(); return jsonify({"id": value.id})


@bp.post("/projects/<int:project_id>/progress/types/<int:type_id>/archive")
@progress_structure_required()
def archive_type(project_id, type_id):
    services.archive_type(_type(project_id, type_id), actor_id=current_user.id); db.session.commit(); return "", 204


@bp.get("/projects/<int:project_id>/progress/types/<int:type_id>")
@progress_read_required()
def type_detail(project_id, type_id):
    value = _type(project_id, type_id); return jsonify({"id": value.id, "name": value.name})


@bp.post("/projects/<int:project_id>/progress/types/<int:type_id>/groups")
@progress_structure_required()
def create_group(project_id, type_id):
    value = services.create_group(progress_type=_type(project_id, type_id), name=request.form.get("name", ""), actor_id=current_user.id); db.session.commit(); return jsonify({"id": value.id}), 201
...
def create_item(project_id, group_id):
    value = services.create_item(group=_group(project_id, group_id), name=request.form.get("name", ""), unit=request.form.get("unit", ""), planned_quantity=request.form.get("planned_quantity", 0), opening_quantity=request.form.get("opening_quantity", 0), actor_id=current_user.id); db.session.commit(); return jsonify({"id": value.id}), 201
...
def item_detail(project_id, item_id):
    item = _item(project_id, item_id); return jsonify({"id": item.id, "name": item.name})
...
def create_entry(project_id, item_id):
    value = services.create_entry(item=_item(project_id, item_id), report_date=request.form.get("report_date"), quantity=request.form.get("quantity"), note=request.form.get("note"), actor_id=current_user.id); db.session.commit(); return jsonify({"id": value.id}), 201
...
def chart_data(project_id, type_id):
    value = _type(project_id, type_id); return jsonify({"type": value.name, "percent": str(services.type_percent(value) or 0)})
