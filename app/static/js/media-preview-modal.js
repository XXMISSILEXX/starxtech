(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (event) => { const trigger = event.target.closest("[data-media-preview-url], [data-media-preview-trigger]"); if (!trigger) return; event.preventDefault(); open({url: trigger.dataset.previewEndpoint || trigger.dataset.mediaPreviewUrl, name: trigger.dataset.filename || trigger.dataset.mediaPreviewName, downloadUrl: trigger.dataset.downloadEndpoint || trigger.dataset.mediaPreviewDownload, type: trigger.dataset.mediaType || trigger.dataset.mediaPreviewType || "image", imageTools: trigger.dataset.enableImageTools === "true" || trigger.dataset.mediaPreviewTools === "1"}); });
