(() => {
  ...
  const label = (principal) => principal.label || '';
  ...
  function renderResults() { if (editing) return; const term = search.value.trim().toLocaleLowerCase(); if (!term) { results.classList.add('d-none'); results.replaceChildren(); return; } const matches = principals.filter((principal) => principal.label.toLocaleLowerCase().includes(term)).slice(0, 8); results.replaceChildren(...matches.map((principal) => { const button = document.createElement('button'); button.type = 'button'; button.className = 'list-group-item list-group-item-action'; button.setAttribute('role', 'option'); button.innerHTML = `<span class="d-block fw-semibold">${escapeHtml(principal.label)}</span><small class="text-muted">${escapeHtml(principal.type === 'user' ? 'Người dùng' : 'Vai trò')}</small>`; button.addEventListener('click', () => choosePrincipal(principal)); return button; })); results.classList.toggle('d-none', matches.length === 0); search.setAttribute('aria-expanded', String(matches.length > 0)); }
