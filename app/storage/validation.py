...
def validate_file_metadata(filename, mime_type, size, checksum_sha256=None):
    filename = (filename or "").strip()
    mime_type = (mime_type or "").strip().lower()
    ext = PurePath(filename).suffix.lower().lstrip(".")
    if not filename or not ext or ext not in ALLOWED_FILES:
        raise StorageValidationError("Loại file không được hỗ trợ.")
    expected_mime, category = ALLOWED_FILES[ext]
    if not _mime_matches(expected_mime, mime_type, ext):
    ...
    if size > _max_size_bytes(category):
    ...
    return {"filename": filename, "mime_type": expected_mime if ext != "wav" else mime_type, "file_ext": "jpg" if ext == "jpeg" else ext, "file_size": size, "checksum_sha256": checksum_sha256 or None, "category": category}
...
def _max_size_bytes(category):
    name = {"image": "STORAGE_MAX_IMAGE_SIZE_MB", "document": "STORAGE_MAX_DOCUMENT_SIZE_MB", "video": "STORAGE_MAX_VIDEO_SIZE_MB", "audio": "STORAGE_MAX_AUDIO_SIZE_MB"}[category]
